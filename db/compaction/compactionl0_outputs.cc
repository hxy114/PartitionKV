...
void CompactionL0Outputs::NewBuilder(const TableBuilderOptions& tboptions) {
  builder_.reset(NewTableBuilder(tboptions, file_writer_.get()));
}

Status CompactionL0Outputs::Finish(
...
IOStatus CompactionL0Outputs::WriterSyncClose(const Status& input_status,
                                            SystemClock* clock,
...
bool CompactionL0Outputs::UpdateFilesToCutForTTLStates(
...
size_t CompactionL0Outputs::UpdateGrandparentBoundaryInfo(
    const Slice& internal_key) {
...
uint64_t CompactionL0Outputs::GetCurrentKeyGrandparentOverlappedBytes(
...
bool CompactionL0Outputs::ShouldStopBefore(const CompactionL0Iterator& c_iter) {
  assert(c_iter.Valid());
...
Status CompactionL0Outputs::AddRangeDels(
...
void CompactionL0Outputs::FillFilesToCutForTtl() {
  if (compaction_->immutable_options()->compaction_style !=
          kCompactionStyleLevel ||
  ...
  //printf("compaction_->num_input_levels() - 2:%zu \n",compaction_->num_input_levels() - 2);
