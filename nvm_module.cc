...
const size_t PM_META_NODE_SIZE=64;//meta node size
const size_t PM_LOG_HEAD_SIZE=128;//pm log head size  ，actually 28 bytes
const size_t PM_LOG_SIZE=64*1024*1024UL;
const size_t PERSIST_SIZE=4*1024*1024;//no use
const size_t PM_META_NODE_NUMBER=60;
const size_t PM_LOG_NUMBER=128;
//const size_t PM_LOG_NUMBER=200;
...
const uint64_t PRE_SPLIT=10;//no use
const uint64_t SPLIT=10;

const uint64_t PRE_MERGE=4;//no use
const uint64_t MERGE=4;

const uint64_t K=7;//no use
const uint64_t PRE_SPLIT_NUMBER=3;//no use
const uint64_t PRE_MERGE_NUMBER=3;//no use
...
const double NEW_SPLIT=1.0/60*2;//no use
const double NEW_MERGE=1.0/60/2;//no use
...
NvmManager::NvmManager (bool is_recover_){
  ...
  if(map_len!=PM_SIZE){
    printf("map_len:%zu, PM_size:%zu",map_len,PM_SIZE);
  ...
  if(is_recover_){
    //TODO recover
    for(size_t i=0;i<PM_META_NODE_NUMBER;i++){
